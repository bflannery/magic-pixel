  ...
  if (typeof Scribe === 'undefined') {
    ...
    ;(function (Scribe) {
      ...
      Handler.prototype.push = function (f) {
        ...
        // console.log({ pushF: f, handlers: this.handlers })
      }
        ...
        if (this.options.trackClicks) {
          Events.onready(function () {
            ...
            Events.onevent(document.body, 'click', true, function (e) {
              ...
              if (
                !ArrayUtil.exists(ancestors, function (e) {
                  return e.tagName === 'A'
                })
              ) {
                self.track('click', {
      ...
      Scribe.prototype.getPath = function (type) {
        var now = new Date()
        var rootNode = this.context.userId
          ? this.options.breakoutUsers
            ? '/users/' + this.context.userId + '/'
            : '/users/'
          : this.options.breakoutVisitors
          ? '/visitors/' + this.context.visitorId + '/'
          : '/visitors/'
        var dateNode
        ...
        if (/daily|day/.test(this.options.bucket)) {
          dateNode =
            now.getUTCFullYear() +
            '-' +
            Util.padLeft(now.getUTCMonth(), 2) +
            '-' +
            Util.padLeft(now.getUTCDate(), 2) +
            '/'
      ...
      Scribe.prototype._sendOutbox = function () {
        for (var i = 0; i < this.outbox.length; i++) {
          ...
          if (ArrayUtil.contains(['redirect', 'formSubmit'], event)) {
            message.value.target = Util.jsonify(
              Util.merge(message.value.target || {}, { url: Util.parseUrl(document.location) }),
            )
          }
      ...
      Scribe.prototype.trackLater = function (name, props) {
        console.log('Track Later', { name, props })
...
ScribeEventPublicTracker.prototype.tracker = function (info) {
  MP_EVENT_TRACKER.trackScribeEvent(info)
}

// Initialize the tracker
console.debug('Scribe: Initializing Scribe')

var MP_EVENT_TRACKER = window.MP_EVENT_TRACKER
